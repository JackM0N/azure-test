...
if (isset($_SESSION["zalogowany"]) && ($_SESSION["zalogowany"]) == false || (!isset($_SESSION["zalogowany"])) || (!isset($_SESSION["uzytkownik"]))) {
    ...
    || !isset($_GET["Cena"]) || (trim($_GET["Cena"]) == "" || !is_numeric($_GET["Cena"])) || !trim(preg_match("/^\d+(?:\.\d{1,4})?$/" ,$_GET["Cena"]))){
        ...
        if($polaczenie == false){
            ...
            if($DataZakonczenia == ''){
            $komenda_sql = "EXECUTE dbo.Zlecenie_Modyfikuj $IdZlecenie, $Brygada, $Status, $Klient, $Obiekt, '$DataRozpoczecia', '$TerminRealizacji', NULL, $Cena, $Uwagi;";
            }else{
            $komenda_sql = "EXECUTE dbo.Zlecenie_Modyfikuj $IdZlecenie, $Brygada, $Status, $Klient, $Obiekt, '$DataRozpoczecia', '$TerminRealizacji', '$DataZakonczenia', $Cena, $Uwagi;";
            }
            $rezultat = sqlsrv_query($polaczenie, $komenda_sql);        
            
            if($rezultat == false){
                print("<p class='msg error'> Edycja danych zlecenia o identyfikatorze: <strong>$IdZlecenie</strong> w bazie nie powiodła się.</p>");
            }else{
                print("<p class='msg success'> Dane zlecenia o identyfikatorze: <strong>$IdZlecenie</strong> zostały zapisane w bazie.</p>");
                print("<p><a href='zlecenie_uzytk_tabela.php' class='bck'>Powrót do wykazu brygad</a></p>");
