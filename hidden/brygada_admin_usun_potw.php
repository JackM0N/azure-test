...
if (isset($_SESSION["zalogowany"]) && ($_SESSION["zalogowany"]) == false || (!isset($_SESSION["zalogowany"])) || (!isset($_SESSION["uzytkownik"]))) {
    ...
    if(!isset($_GET["IdBrygada"]) || (trim($_GET["IdBrygada"]) == "") || !is_numeric($_GET["IdBrygada"]) || !trim(preg_match('/^[0-9]+$/' ,$_GET["IdBrygada"]))){
        ...
        if($polaczenie == false){
            ...
            $komenda_sql = "EXECUTE dbo.Brygada_Usun $IdBrygada";

            $rezultat = sqlsrv_query($polaczenie, $komenda_sql);        
            
            if($rezultat == false){
                print("<p class='msg error'> Usunięcie danych brygady o identyfikatorze <strong>$IdBrygada</strong> w bazie nie powiodła się.</p>");
            }else{
                print("<p class='msg success'> Dane brygady o identyfikatorze <strong>$IdBrygada</strong> zostały usunięte z bazy.</p>");
