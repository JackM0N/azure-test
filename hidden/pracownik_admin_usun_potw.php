...
if (isset($_SESSION["zalogowany"]) && ($_SESSION["zalogowany"]) == false || (!isset($_SESSION["zalogowany"])) || (!isset($_SESSION["uzytkownik"]))) {
    ...
    if(!isset($_GET["IdPracownik"]) || (trim($_GET["IdPracownik"]) == "") || !is_numeric($_GET["IdPracownik"])){
        print("<p class='msg error'> Nie można usunąć danych pracownika, ponieważ są one niekompletne lub błędne</p>");
        ...
        if($polaczenie == false){
            ...
            $komenda_sql = "DELETE dbo.Pracownik
                            WHERE IdPracownik = $IdPracownik;";

            $rezultat = sqlsrv_query($polaczenie, $komenda_sql);        
            
            if($rezultat == false){
                print("<p class='msg error'> Usunięcie danych pracownika o identyfikatorze <strong>$IdPracownik</strong> w bazie nie powiodła się.</p>");
            }else{
                print("<p class='msg success'> Dane pracownika o identyfikatorze <strong>$IdPracownik</strong> zostały usunięte z bazy.</p>");
