...
if (isset($_SESSION["zalogowany"]) && ($_SESSION["zalogowany"]) == false || (!isset($_SESSION["zalogowany"])) || (!isset($_SESSION["uzytkownik"]))) {
    ...
    if(!isset($_GET["IdZlecenie"]) || (trim($_GET["IdZlecenie"]) == "") || !is_numeric($_GET["IdZlecenie"]) || !trim(preg_match('/^[0-9]+$/', $_GET["IdZlecenie"]))){
        ...
        $dane_polaczenia = array("Database" => "P15_C10", "CharacterSet" => "UTF-8");
        ...
        if($polaczenie == false){
            ...
            if(sqlsrv_has_rows($zbior_wierszy) == false){
                ...
                print("<table class='szczegoly'>
                    <thead>
                        <tr class='uslugi'>
                            <td><a href='zlecenie_admin_szczegoly.php?sort=UslugaNazwa' class='head'>Nazwa usługi</a></td>
                            <td><a href='zlecenie_admin_szczegoly.php?sort=Ilosc' class='head'>Ilość</a></td>
                            <td><a href='zlecenie_admin_szczegoly.php?sort=Cena' class='head'>Wartość usług</a></td>
                        </tr>
                    </thead>
                    <tbody>");
        ...
        print("</tbody>
                ...
                print("<table class='szczegoly'>
                    <thead>
                        <tr class='uslugi'>
                            <td><a href='zlecenie_admin_szczegoly.php?sort=Nazwa' class='head'>Nazwa matriału</a></td>
                            <td><a href='zlecenie_admin_szczegoly.php?sort=CenaZaplacona' class='head'>Cena</a></td>
                            <td><a href='zlecenie_admin_szczegoly.php?sort=WykorzystanieMaterialu' class='head'>Wykorzystanie materiału</a></td>
                        </tr>
                    </thead>
                    <tbody>");
