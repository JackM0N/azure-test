...
if (isset($_SESSION["zalogowany"]) && ($_SESSION["zalogowany"]) == false || (!isset($_SESSION["zalogowany"])) || (!isset($_SESSION["uzytkownik"]))) {
    ...
    || !isset($_GET["AdresEmail"]) || (trim($_GET["AdresEmail"]) == "")){
        ...
        if($polaczenie == false){
            ...
            $komenda_sql = "INSERT dbo.Pracownik
                            (IdPracownik, Imie, Nazwisko, Specjalizacja, NumerTelefonu, AdresEmail, Uwagi)
                            VALUES
                            ($IdPracownik, '$Imie', '$Nazwisko', '$Specjalizacja', '$NumerTelefonu', '$AdresEmail', '$Uwagi');";
            ...
            if($rezultat == false){
                print("<p class='msg error'> Zapisanie danych pracownika <strong>$Imie $Nazwisko</strong> w bazie nie powiodło się.</p>");
            }else{
