...
if (isset($_SESSION["zalogowany"]) && ($_SESSION["zalogowany"]) == false || (!isset($_SESSION["zalogowany"])) || (!isset($_SESSION["uzytkownik"]))) {
    ...
    || !isset($_GET["Brygadzista"]) || (trim($_GET["Brygadzista"]) == "0" || !is_numeric($_GET["Brygadzista"])) || !trim(preg_match('/^[0-9]+$/' ,$_GET["Brygadzista"]))
    ...
    || !isset($_GET["Specjalizacja"]) || (trim($_GET["Specjalizacja"]) == "") || !trim(preg_match('/^[a-zA-ZąćęłńóśźżĄĆĘŁŃÓŚŹŻ]+$/u',$_GET["Specjalizacja"]))){
        ...
        if($polaczenie == false){
            ...
            $komenda_sql = "EXECUTE dbo.Brygada_Wstaw $IdBrygada, $Brygadzista, $LiczbaPracownikow, '$Specjalizacja', $Uwagi;";

            $rezultat = sqlsrv_query($polaczenie, $komenda_sql);        
            
            if($rezultat == false){
                print("<p class='msg error'> Zapisanie danych brygady o identyfikatorze: <strong>$IdBrygada</strong> w bazie nie powiodło się.</p>");
